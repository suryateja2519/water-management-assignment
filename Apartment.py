...
class Apartment:
    def __init__(self, bhk_type, corporation_is_to_borewell_ratio):
        ...
        if bhk_type == Variables.TWO_BHK:
            self.total_no_of_tenants = Variables.TENTANTS_IN_2_BHK
        else:
            self.total_no_of_tenants = Variables.TENTANTS_IN_3_BHK

        self.monthly_water_consumption_allowance = self.total_no_of_tenants * \
            Variables.CONSUMPTION_OF_A_PERSON_PER_DAY*Variables.NO_OF_DAYS_IN_A_MONTH
    ...
    def get_cost_by_corporate(self):
        cost = self.coorporation_water_allocated*Variables.CORPORATION_WATER_RATE
        return cost

    def get_cost_by_borewell(self):
        cost = self.borewell_water_allocated*Variables.BOREWELL_WATER_RATE
        return cost

    def get_cost_by_tank(self, water_quantity):
        total = 0
        brackets = [
            [Variables.TANKER_SLAB_0_TO_500L_MAX,
                Variables.TANKER_SLAB_0_TO_500L_RATE],
            [Variables.TANKER_SLAB_501_TO_1500L_MAX-Variables.TANKER_SLAB_0_TO_500L_MAX,
                Variables.TANKER_SLAB_501_TO_1500L_RATE],
            [Variables.TANKER_SLAB_1501_TO_3000L_MAX-Variables.TANKER_SLAB_501_TO_1500L_MAX,
                Variables.TANKER_SLAB_1501_TO_3000L_RATE]
        ]
        ...
        if water_quantity != 0:
            total += (water_quantity)*Variables.TANKER_SLAB_3001_PLUS_RATE
        return total
